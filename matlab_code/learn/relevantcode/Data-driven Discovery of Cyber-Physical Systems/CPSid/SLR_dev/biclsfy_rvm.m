...
[ww, ix_eff, W, AX] = slr_learning_var2(b_train, Xtr, 'nlearn', Nlearn, 'nstep', Nstep, 'amax', AMAX);
AXall = AX;
ix_eff_all = ix_eff;
...
% Training Result
%----------------------
[t_train_est, Ptr] = calc_label(Xtr, [zeros(length(ww),1),ww]);  % {1,2}
...
if isempty(ix_eff)
    ...
    [t_test_est, Pte] = calc_label(Xte, [zeros(length(ix_eff),1), ww(ix_eff)]); % {1,2}
end
