function [ww, ix_eff_all, errTable_tr, errTable_te, parm, AXall, Ptr, Pte, pairs] =...
    muclsfy_slrvarovo2(x_train, t_train, x_test, t_test, varargin)
...
parm = finputcheck(varargin, ...
    {'scale_mode'  , 'string' , {'all','each','stdall','stdeach','none','medeach'}, 'each';...
     'mean_mode'   , 'string' , {'all','each','none','medeach'}, 'each';...
     'ax0'         , 'real'   ,  [],  [];...
     'nlearn'      , 'integer',  [1 inf],  1000;...
     'nstep'       , 'integer',  [1 inf],  100;...
     'amax'        , 'real'   ,  [0 inf],  1e8;...
...
if isempty(x_train),
    fprintf('No training data is given !!');
    ww = [];
    ix_eff_all = [];
    errTable_tr = [];
    errTable_te = [];
...
if isempty(parm.base)
    ...
    [x_train, scale, base] = normalize_feature(x_train, parm.scale_mode, parm.mean_mode, parm.scale, parm.base);
    [x_test, scale, base] = normalize_feature(x_test, parm.scale_mode, parm.mean_mode, parm.scale, parm.base);
end
