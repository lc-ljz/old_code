...
% Group-wise sparseness is allowed.
% 
% -- Usage
...
% 2011/10/04 OY   ver2.0 
% * 'group' field is introduced in parm struct in order to allow for group-wise
% sparseness
% 2010/07/07 OY
...
if usebias
    ...
    group = [group; length(unique(group))+1];
else
    Xtr = x_train;
    Xte = x_test;
end
...
if (length(unique(group)) == 1 & usebias == 0) | (length(unique(group)) == 2 & usebias == 1)
    [ww, ix_eff, W, AXall] = slr_learning(b_train, Xtr, @linfun,...
        'nlearn', Nlearn, 'nstep', Nstep,...
        'wdisplay', wdisp_mode, 'wmaxiter', wmaxiter, 'amax', AMAX);
else
    [ww, ix_eff, W, AXall] = slr_learning_group(b_train, Xtr, @linfun, group, ...
        'nlearn', Nlearn, 'nstep', Nstep,...
        'wdisplay', wdisp_mode, 'wmaxiter', wmaxiter, 'amax', AMAX);
end
