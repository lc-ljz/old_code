...
A= library(x,polyorder,memory,basis_function);  %make the dic

%%
switch np
    case 0
        parameter.lambda = [0.001 0.05];   % the lambda of z in algorithm 1.
...
%%
ture_sys1 = [1 ;0;0;0;0;0];
ture_sys2 = [-1;0;0;0;0;0];
...
for k = 1:size(sys,2)
    ...
    if abs((sys(:,k)-ture_sys1))<0.05
        judge(idx_sys{k}) = 2;
        
        
    elseif abs((sys(:,k)-ture_sys2))<0.05
        judge(idx_sys{k}) = 3;
...
Phi2 = [ones(size(x)) x   exp(10*(sin(x.^2))+10)  log(abs(x))./sin(x)  x.^4 x.^2];
