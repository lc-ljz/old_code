...
function f() {
    ...
    var postFolder = $("#postName").combobox('getValue');
    //时间
    ...
    $.ajax({
        url: '../UploadServlet?postFolder='+postFolder,
        type: "post",
        data: form,
        dataType: "json",
        processData: false,
...
function LoadAccountInfoGridByTree(txtPosName) {
    $('#orderList').datagrid({
        columns: [[
            ...
            // { field: 'txtMrtClassName', title: '资料分类', width: "5%", align: 'center' },
            { field: 'txtLrnPath', title: '存储路径', width: "25%", align: 'center',formatter:formatFile },
            ...
            { field: 'txtPosName', title: '目录编号', width: "5%", align: 'center' },
            // { field: 'cFirstClass', title: '一级', width: "5%", align: 'center' },
            // { field: 'cSecondSlass', title: '二级', width: "5%", align: 'center' },
        ...
        singleSelect: true,
        rownumbers: true,
        url: '../SelectTaskListByTreeController?txtPosName='+txtPosName,
        fitColumns: true,
        pagination : true,
        ...
        pageSize : 10,
        pageList : [10,15]//允许表格自动缩放,以适应父容器
    })
}
...
function InitAddCbo() {
    ...
    $('#postName').combobox({
        ...
        onSelect: function (param) {
            var postFolder = param.cFolder;
            LoadPostFolder(postFolder);
        }
...
//////////////////////加载岗位对应文件信息////////////////////
function LoadPostFolder(postFolder) {
    $.ajax({
        type: "POST",
        url: "../GetPostFolderController?postFolder="+postFolder,
        dataType: "json",
        success: function (data) {
                $('#postFolder').combobox('loadData', data);
        },
        error: function () {
            alert('加载失败！');
        }
...
function InitPostFolderCbo() {
    /////////////////////////////////岗位对应文件目录信息/////////////////////////////////////////
    $('#postFolder').combobox({
        valueField: 'cFolderName',
        textField: 'cFolderName',
        onLoadSuccess: function () { //加载完成后,设置选中第一项
            var val = $(this).combobox("getData");
            for (var item in val[0]) {
                if (item == "cFolderName") {
...
function LoadAccountInfoGrid() {
    ...
    $('#orderList').datagrid({
        ...
        singleSelect: true,
        rownumbers: true,
         url: '../SelectTaskListController?myEquipmentName='+myEquipmentName,
        fitColumns: true,
        pagination : true,
        onClickRow:function (rowIndex, rowData){
            document.getElementById("Remark").value=rowData.txtRemark;
