...
def reconstruct(seqData, mean, std):  # 标准化
...
class PickleDataLoad(object):
    ...
    def batchify(self, args, data, bsz):
        ...
        trimmed_data = data.narrow(0, 0, nbatch * bsz)  # 取出 nbatch * bsz 行的数据
        # transpose()进行转置操作, contiguous()断开这两个变量之间的依赖  torch.Size([nbatch, bsz, 2])
